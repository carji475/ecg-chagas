...
def train(ep, dataload):
    ...
    train_bar = tqdm(initial=0, leave=True, total=math.ceil(len(dataload)/args.batch_size),
                     desc=train_desc.format(epoch=ep, loss=0), position=0)  # progress bar
...
def eval(ep, dataload):
    ...
    eval_bar = tqdm(initial=0, leave=True, total=math.ceil(len(dataload)/args.batch_size),
                    desc=eval_desc.format(epoch=ep, loss=0), position=0)
    eval_outputs = np.zeros(dataload.dset.in_chagas[dataload.start_idx:dataload.end_idx].sum())
    ...
    for traces, diagnoses in dataload:
        ...
        with torch.no_grad():
            ...
            # store output
            end = min(start + len(traces), eval_outputs.size)
            eval_outputs[start:end] = torch.nn.Sigmoid()(model_output).detach().cpu().numpy().flatten()
...
if __name__ == "__main__":
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimiser, patience=args.patience,
